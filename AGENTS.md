...
- **Tool registration beta compat** (`index.ts` `addTool`): the tagged `@opencode/plugin` types are `tools.add(tool)` (an older beta documented the positional `add(name, tool, options?)`). The types have settled on one arity, so the `Function.length` shim is gone: `addTool` only guards that `add` exists and forwards the definition object — don't reintroduce the arity shim. The `prune` tool is registered with **`options: { codemode: false, permission: "prune" }`** (not `codemode: true`): `packages/core/src/tool.ts` only emits `codemode === false` tools as native definitions, so the full `PRUNE_RANGE` boundary-ID rules ship on every request; `codemode: true` — or leaving it unset, the platform default — folds the tool into the Code Mode `execute` catalog, whose inline listings truncate the first line to 120 chars (`packages/core/src/codemode/catalog.ts`). Note the registration `options` are set **on the definition object** inside `prune-tool.ts`, not by `addTool`. Do not "restore" `codemode: true`.
