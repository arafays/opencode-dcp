...
export function resolveOptions(
  ...
  const tuiInput = (typeof input.tui === "object" && input.tui !== null ? input.tui : {}) as Record<
    string,
    unknown
  >;
  ...
  return {
    ...
    protectedFilePatterns:
      mergeStringArray(input.protectedFilePatterns) ?? DEFAULTS.protectedFilePatterns,
    tui: {
    ...
    strategies: {
      ...
      purgeErrors: {
        ...
        turns:
          typeof purgeInput.turns === "number" && purgeInput.turns >= 1
            ? Math.floor(purgeInput.turns)
            : 4,
